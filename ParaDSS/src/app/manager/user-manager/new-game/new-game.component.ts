...
export class NewGameComponent implements OnInit {
  ...
  private maxLimitGames: number = 10;
  ...
  onSubmit() {
    ...
    */
    ...
    if (this.dataBase.connect.getArrayGames().length === this.maxLimitGames)
    {
      console.log("Limite maximo de Juegos alcanzados");
      window.alert("Limite maximo de Juegos alcanzados");
    }

    let gameBD = this.dataBase.connect.getGame(this.newGame);

    if (gameBD === null || gameBD === undefined) 
    {
      this.dataBase.connect.add(this.newGame);
